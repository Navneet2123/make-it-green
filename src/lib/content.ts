...
export type Item =
  ...
  | { kind: 'locate'; prompt: string; locator: string; target: string; explain: string }
...
export const LEVELS: Level[] = [
  ...
  },
  {
    ...
    story: 'A robot only does what you tell it, in the order you tell it. That list of steps is a script.',
    items: [
      {
  ...
  },
  {
    ...
    items: [
      { kind: 'locate', prompt: 'Tap the element this locator points to:', locator: '#login', target: 'login', explain: '# means "the element with this id". The Log in button has id="login".' },
      { kind: 'locate', prompt: 'This one is XPath. Tap what it finds:', locator: "//input[@name='password']", target: 'password', explain: '// = anywhere on the page · input = an input box · [@name=\'password\'] = whose name is password.' },
      { kind: 'locate', prompt: 'Last one. Tap what it finds:', locator: "//a[text()='Sign up']", target: 'signup', explain: 'A link (<a>) whose text is exactly "Sign up". Text locators break if someone renames the link, so ids are safer.' },
    ],
  },
  {
    ...
    items: [
      ...
      { kind: 'judge', expected: '"Welcome, Sam"', actual: '"Welcome, sam"', pass: false, explain: 'Robots are literal. A lowercase s is a different string. FAIL.' },
  ...
  },
  {
    ...
    items: [
      ...
      {
        kind: 'fix', script: ['OPEN the website', 'TYPE username "sam"', 'TYPE password "robot123"', 'CLICK #login', 'CHECK dashboard'],
        brokenLine: 4, output: 'Assertion failed at step 5: page was still loading', fixes: ['CLICK #login twice', 'WAIT for the page to load', 'Delete step 5'], answer: 1,
